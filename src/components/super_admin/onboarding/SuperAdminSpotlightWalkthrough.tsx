...
import { SUPER_ADMIN_WALKTHROUGH_STORAGE_KEY_PREFIX } from "./shared";
...
const spotlightSteps: SpotlightStep[] = [
  {
    id: "dashboard-hero",
    route: "/home/super_admin/dashboard",
    selector: '[data-super-admin-tour="dashboard-hero"]',
    navSelector: '[data-super-admin-tour="nav-dashboard"]',
    title: "Super admin dashboard",
    description:
      "This is your control center for staffing coverage, workflow movement, and system-wide oversight.",
  },
  {
    id: "officers-page",
    route: "/home/super_admin/officers",
    selector: '[data-super-admin-tour="officers-header"]',
    navSelector: '[data-super-admin-tour="nav-officers"]',
    title: "Officer management",
    description:
      "Create and manage evaluator, inspector, treasurer, and admin accounts from this workspace.",
  },
  {
    id: "permits-page",
    route: "/home/super_admin/permits",
    selector: '[data-super-admin-tour="permits-header"]',
    navSelector: '[data-super-admin-tour="nav-permits"]',
    title: "Permit management",
    description:
      "Use this page to control the permits available in the system and open their builders.",
  },
  {
    id: "templates-page",
    route: "/home/super_admin/permit-templates",
    selector: '[data-super-admin-tour="permit-templates-header"]',
    navSelector: '[data-super-admin-tour="nav-permit-templates"]',
    title: "Permit templates",
    description:
      "Upload, map, activate, and maintain document templates used across permit workflows.",
  },
  {
    id: "process-page",
    route: "/home/super_admin/process",
    selector: '[data-super-admin-tour="process-header"]',
    navSelector: '[data-super-admin-tour="nav-process"]',
    title: "Inspection process",
    description:
      "Define the inspection department order here so permit routing follows the right workflow steps.",
  },
  {
    id: "audit-history-page",
    route: "/home/super_admin/audit-history",
    selector: '[data-super-admin-tour="audit-history-header"]',
    navSelector: '[data-super-admin-tour="nav-audit-history"]',
    title: "Workflow audit history",
    description:
      "Review system-wide workflow events, filter activity, and inspect record details in one place.",
  },
  {
    id: "account-settings-page",
    route: "/home/super_admin/account",
    selector: '[data-super-admin-tour="account-settings-shell"]',
    navSelector: '[data-super-admin-tour="nav-account-settings"]',
    title: "Account settings",
    description:
      "Manage your super admin profile, security settings, and BPLO branding controls here.",
  },
];

type SuperAdminSpotlightWalkthroughProps = {
...
export default function SuperAdminSpotlightWalkthrough({
  accountIdentifier,
  isSuperAdmin,
  isSuperAdminRoute,
}: SuperAdminSpotlightWalkthroughProps) {
  ...
  const startSignal = useSuperAdminWalkthroughStore(
  ...
  const storageKey = useMemo(() => {
    ...
    return `${SUPER_ADMIN_WALKTHROUGH_STORAGE_KEY_PREFIX}:${normalizedIdentifier}`;
  }, [accountIdentifier]);
  ...
  const clearSpotlightedElements = () => {
    for (const element of spotlightedElementsRef.current) {
      delete element.dataset.superAdminWalkthroughSpotlight;
    }
  ...
  const markSpotlightedElement = (
    ...
    element.dataset.superAdminWalkthroughSpotlight = kind;
  };
  ...
  useEffect(() => {
    if (!isSuperAdmin || !isSuperAdminRoute || !storageKey || isOpen) {
      return;
    }

    // Auto-start once per account unless walkthrough was already completed.
  ...
  }, [isSuperAdmin, isSuperAdminRoute, storageKey, isOpen]);

  useEffect(() => {
    if (!startSignal) return;
    if (!isSuperAdmin || !isSuperAdminRoute) return;
    startWalkthrough();
  }, [startSignal, isSuperAdmin, isSuperAdminRoute]);

  useEffect(() => {
    if (!isOpen || !isSuperAdminRoute) return;

    if (location.pathname !== activeStep.route) {
      // Navigate to target route first; spotlight waits for target DOM to resolve.
  ...
  useEffect(() => {
    ...
    const refreshTarget = () => {
      ...
      // Resolve both content and nav targets so overlay works across layouts.
  ...
  if (!isOpen || !isSuperAdmin || !isSuperAdminRoute) {
    return null;
  }
  ...
  return (
    <>
      ...
      <style>{`
        [data-super-admin-walkthrough-spotlight="target"] {
          position: relative !important;
          z-index: 119 !important;
          isolation: isolate;
        }

        [data-super-admin-walkthrough-spotlight="nav"] {
          position: relative !important;
