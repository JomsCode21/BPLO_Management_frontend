...
import { BPLO_ADMIN_WALKTHROUGH_STORAGE_KEY_PREFIX } from "./shared";
...
const spotlightSteps: SpotlightStep[] = [
  {
    id: "dashboard-hero",
    route: "/home/bplo_admin/dashboard",
    selector: '[data-bplo-admin-tour="dashboard-hero"]',
    navSelector: '[data-super-admin-tour="nav-dashboard"]',
    title: "BPLO admin dashboard",
    description:
      "Start here for a full snapshot of BPLO workload across validity tracking, inspection routing, and approval queues.",
  },
  {
    id: "permit-validity-page",
    route: "/home/bplo_admin/permit-validity",
    selector: '[data-bplo-admin-tour="permit-validity-header"]',
    navSelector: '[data-super-admin-tour="nav-permit-validity"]',
    title: "Permit validity",
    description:
      "Monitor released permits by validity status and open full routed details from this registry.",
  },
  {
    id: "inspection-request-page",
    route: "/home/bplo_admin/inspection-request",
    selector: '[data-bplo-admin-tour="inspection-request-header"]',
    navSelector: '[data-super-admin-tour="nav-inspection-request"]',
    title: "Inspection request queue",
    description:
      "Handle applications routed for inspection and submit BPLO inspection request decisions.",
  },
  {
    id: "permit-approval-page",
    route: "/home/bplo_admin/permit-approval",
    selector: '[data-bplo-admin-tour="permit-approval-header"]',
    navSelector: '[data-super-admin-tour="nav-permit-approval"]',
    title: "Permit approval",
    description:
      "Finalize permit approval decisions and trigger generated permit actions for approved records.",
  },
  {
    id: "permit-release-page",
    route: "/home/bplo_admin/permit-release",
    selector: '[data-bplo-admin-tour="permit-release-header"]',
    navSelector: '[data-super-admin-tour="nav-permit-release"]',
    title: "Permit release",
    description:
      "Review approved records ready for release and print original generated permit copies.",
  },
  {
    id: "payment-analytics-page",
    route: "/home/bplo_admin/payment-analytics",
    selector: '[data-bplo-admin-tour="payment-analytics-header"]',
    navSelector: '[data-super-admin-tour="nav-payment-analytics"]',
    title: "Payment Received",
    description:
      "Review paid assessment totals per permit type and drill into payer records from this page.",
  },
  {
    id: "fee-assessment-page",
    route: "/home/bplo_admin/fee-assessment",
    selector: '[data-bplo-admin-tour="fee-assessment-header"]',
    navSelector: '[data-super-admin-tour="nav-fee-assessment"]',
    title: "Fee assessment setup",
    description:
      "Define and maintain BPLO fee templates per permit type for downstream payment processing.",
  },
  {
    id: "audit-history-page",
    route: "/home/bplo_admin/audit-history",
    selector: '[data-bplo-admin-tour="audit-history-header"]',
    navSelector: '[data-super-admin-tour="nav-audit-history"]',
    title: "Workflow history",
    description:
      "Filter BPLO-specific workflow events and export a PDF trail for compliance reporting.",
  },
  {
    id: "account-settings-page",
    route: "/home/bplo_admin/account",
    selector: '[data-bplo-admin-tour="account-settings-shell"]',
    navSelector: '[data-super-admin-tour="nav-account-settings"]',
    title: "Account settings",
    description:
      "Update your account profile and security credentials for your BPLO admin login.",
  },
];

type BploAdminSpotlightWalkthroughProps = {
  accountIdentifier: string;
  isBploAdmin: boolean;
  isBploAdminRoute: boolean;
};
...
export default function BploAdminSpotlightWalkthrough({
  accountIdentifier,
  isBploAdmin,
  isBploAdminRoute,
}: BploAdminSpotlightWalkthroughProps) {
  ...
  const startSignal = useBploAdminWalkthroughStore(
  ...
  const storageKey = useMemo(() => {
    ...
    return `${BPLO_ADMIN_WALKTHROUGH_STORAGE_KEY_PREFIX}:${normalizedIdentifier}`;
  }, [accountIdentifier]);
  ...
  const clearSpotlightedElements = () => {
    for (const element of spotlightedElementsRef.current) {
      delete element.dataset.bploAdminWalkthroughSpotlight;
    }
  ...
  const markSpotlightedElement = (
    ...
    element.dataset.bploAdminWalkthroughSpotlight = kind;
  };
  ...
  useEffect(() => {
    if (!isBploAdmin || !isBploAdminRoute || !storageKey || isOpen) {
      return;
    }
  ...
  }, [isBploAdmin, isBploAdminRoute, storageKey, isOpen]);

  useEffect(() => {
    if (!startSignal) return;
    if (!isBploAdmin || !isBploAdminRoute) return;
    startWalkthrough();
  }, [startSignal, isBploAdmin, isBploAdminRoute]);

  useEffect(() => {
    if (!isOpen || !isBploAdminRoute) return;
  ...
  }, [activeStep.route, isOpen, isBploAdminRoute, location.pathname, navigate]);
  ...
  if (!isOpen || !isBploAdmin || !isBploAdminRoute) {
    return null;
  }
  ...
  return (
    <>
      ...
      <style>{`
        [data-bplo-admin-walkthrough-spotlight="target"] {
          position: relative !important;
          z-index: 119 !important;
          isolation: isolate;
        }

        [data-bplo-admin-walkthrough-spotlight="nav"] {
          position: relative !important;
