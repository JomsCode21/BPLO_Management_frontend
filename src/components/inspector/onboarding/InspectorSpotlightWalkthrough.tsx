...
import { INSPECTOR_WALKTHROUGH_STORAGE_KEY_PREFIX } from "./shared";
...
const spotlightSteps: SpotlightStep[] = [
  {
    id: "dashboard-hero",
    route: "/home/inspector/dashboard",
    selector: '[data-inspector-tour="dashboard-hero"]',
    navSelector: '[data-super-admin-tour="nav-dashboard"]',
    title: "Inspector dashboard",
    description:
      "This is your inspection command view for queue pressure, schedule load, and result outcomes.",
  },
  {
    id: "inspection-requests-page",
    route: "/home/inspector/inspection-requests",
    selector: '[data-inspector-tour="inspection-requests-header"]',
    navSelector: '[data-super-admin-tour="nav-inspection-requests"]',
    title: "Inspection requests",
    description:
      "Review newly assigned applications and set the first schedule for each inspection request.",
  },
  {
    id: "schedule-inspection-page",
    route: "/home/inspector/schedule-inspection",
    selector: '[data-inspector-tour="schedule-inspection-header"]',
    navSelector: '[data-super-admin-tour="nav-schedule-inspection"]',
    title: "Schedule inspection",
    description:
      "Manage your calendar and adjust dates for active inspections from this scheduling workspace.",
  },
  {
    id: "inspection-assessment-page",
    route: "/home/inspector/inspection-assessment",
    selector: '[data-inspector-tour="inspection-assessment-header"]',
    navSelector: '[data-super-admin-tour="nav-inspection-assessment"]',
    title: "Inspection assessment",
    description:
      "Submit inspection results and generate the proper certificate path for each completed visit.",
  },
  {
    id: "certificates-page",
    route: "/home/inspector/certificates",
    selector: '[data-inspector-tour="certificates-header"]',
    navSelector: '[data-super-admin-tour="nav-certificates"]',
    title: "Certificate templates",
    description:
      "Upload and maintain inspection certificate templates, placeholder mappings, and active versions.",
  },
  {
    id: "permit-release-page",
    route: "/home/inspector/permit-release",
    selector: '[data-inspector-tour="permit-release-header"]',
    navSelector: '[data-super-admin-tour="nav-permit-release"]',
    title: "Permit release",
    description:
      "Track approved records and print original generated permits when they are ready for release.",
  },
  {
    id: "audit-history-page",
    route: "/home/inspector/audit-history",
    selector: '[data-inspector-tour="audit-history-header"]',
    navSelector: '[data-super-admin-tour="nav-audit-history"]',
    title: "Workflow history",
    description:
      "Filter and review inspector workflow events and export a PDF record for your audit trail.",
  },
  {
    id: "account-settings-page",
    route: "/home/inspector/account",
    selector: '[data-inspector-tour="account-settings-shell"]',
    navSelector: '[data-super-admin-tour="nav-account-settings"]',
    title: "Account settings",
    description:
      "Update your profile and password settings to keep your inspector account secure and current.",
  },
];

type InspectorSpotlightWalkthroughProps = {
  accountIdentifier: string;
  isInspector: boolean;
  isInspectorRoute: boolean;
};
...
export default function InspectorSpotlightWalkthrough({
  accountIdentifier,
  isInspector,
  isInspectorRoute,
}: InspectorSpotlightWalkthroughProps) {
  ...
  const startSignal = useInspectorWalkthroughStore(
  ...
  const storageKey = useMemo(() => {
    ...
    return `${INSPECTOR_WALKTHROUGH_STORAGE_KEY_PREFIX}:${normalizedIdentifier}`;
  }, [accountIdentifier]);
  ...
  const clearSpotlightedElements = () => {
    for (const element of spotlightedElementsRef.current) {
      delete element.dataset.inspectorWalkthroughSpotlight;
    }
  ...
  const markSpotlightedElement = (
    ...
    element.dataset.inspectorWalkthroughSpotlight = kind;
  };
  ...
  useEffect(() => {
    if (!isInspector || !isInspectorRoute || !storageKey || isOpen) {
      return;
    }
  ...
  }, [isInspector, isInspectorRoute, storageKey, isOpen]);

  useEffect(() => {
    if (!startSignal) return;
    if (!isInspector || !isInspectorRoute) return;
    startWalkthrough();
  }, [startSignal, isInspector, isInspectorRoute]);

  useEffect(() => {
    if (!isOpen || !isInspectorRoute) return;
  ...
  }, [activeStep.route, isOpen, isInspectorRoute, location.pathname, navigate]);
  ...
  if (!isOpen || !isInspector || !isInspectorRoute) {
    return null;
  }
  ...
  return (
    <>
      ...
      <style>{`
        [data-inspector-walkthrough-spotlight="target"] {
          position: relative !important;
          z-index: 119 !important;
          isolation: isolate;
        }

        [data-inspector-walkthrough-spotlight="nav"] {
          position: relative !important;
