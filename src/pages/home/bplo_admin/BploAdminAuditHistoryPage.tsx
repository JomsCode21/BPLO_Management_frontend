import {
  downloadBploWorkflowAuditPdfApi,
  getBploWorkflowAuditApi,
...
const initialPagination: BploWorkflowAuditPaginationType = {
  page: 1,
...
export default function BploAdminAuditHistoryPage() {
  const [rows, setRows] = useState<BploWorkflowAuditEventType[]>([]);
  const [pagination, setPagination] =
    useState<BploWorkflowAuditPaginationType>(initialPagination);
  ...
  const [selectedEvent, setSelectedEvent] =
    useState<BploWorkflowAuditEventType | null>(null);
  ...
  const loadAudit = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      const response = await getBploWorkflowAuditApi({
        page,
        limit: ITEMS_PER_PAGE,
        permit,
        search: search.trim(),
      ...
      if (!isSilent) {
        ...
        setError(
          apiError?.response?.data?.message ??
            "Failed to load BPLO workflow history.",
        );
      }
    } finally {
  ...
  useEffect(() => {
    ...
    socket.on(ADMIN_WORKFLOW_AUDIT_EVENT, () => {
      void loadAudit({ silent: true });
    });

    return () => {
      socket.off(ADMIN_WORKFLOW_AUDIT_EVENT);
      socket.disconnect();
    };
  }, [loadAudit]);
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      // Debounce typing to avoid issuing a request for every key press.
  ...
  const handleDownloadPdf = async () => {
    ...
    setError(null);
    try {
      const pdfBlob = await downloadBploWorkflowAuditPdfApi({
        permit,
        search: search.trim(),
        dateFrom,
        dateTo,
      ...
      anchor.download = `bplo-workflow-history-${datePart}.pdf`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      // Delay revoke slightly to avoid interrupting browser download handoff.
      window.setTimeout(() => URL.revokeObjectURL(objectUrl), 500);
    } catch (err: unknown) {
      const apiError = err as { response?: { data?: { message?: string } } };
      setError(
        apiError?.response?.data?.message ??
          "Failed to download BPLO audit history PDF.",
      );
    } finally {
      setIsPrinting(false);
    }
  ...
  return (
    <div className="min-h-screen p-2 md:p-6 flex flex-col gap-4">
      <section
        ...
        data-bplo-admin-tour="audit-history-header"
      >
        <h1 className="text-2xl font-black text-[#0F2942]">BPLO Workflow History</h1>
        <p className="mt-2 text-sm font-medium text-gray-600">
          Workflow events scoped to BPLO admin responsibilities.
        </p>
      ...
      <section className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
        {rows.length === 0 ? (
          <div className="py-12 px-5 text-center text-sm font-semibold text-gray-500">
            No BPLO workflow records found for the current filter.
          </div>
        ) : (
      ...
      <WorkflowAuditDetailsModal
        ...
        event={selectedEvent}
        title="BPLO Workflow Event Details"
