import {
  downloadInspectorWorkflowAuditPdfApi,
  getInspectorWorkflowAuditApi,
...
const initialPagination: InspectorWorkflowAuditPaginationType = {
  page: 1,
...
export default function InspectorAuditHistoryPage() {
  const [rows, setRows] = useState<InspectorWorkflowAuditEventType[]>([]);
  const [pagination, setPagination] =
    useState<InspectorWorkflowAuditPaginationType>(initialPagination);
  ...
  const [selectedEvent, setSelectedEvent] =
    useState<InspectorWorkflowAuditEventType | null>(null);
  ...
  const loadAudit = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      try {
        const response = await getInspectorWorkflowAuditApi({
          page,
          limit: ITEMS_PER_PAGE,
          permit,
          search: search.trim(),
        ...
        // Ignore stale responses when page/filters change during request.
        ...
        if (!isSilent) {
          ...
          setError(
            apiError?.response?.data?.message ??
              "Failed to load inspector workflow history.",
          );
        }
      } finally {
        if (requestId !== latestRequestIdRef.current) return;
  ...
  useEffect(() => {
    ...
    socket.on(INSPECTOR_WORKFLOW_AUDIT_EVENT, () => {
      void loadAudit({ silent: true });
    });

    return () => {
      socket.off(INSPECTOR_WORKFLOW_AUDIT_EVENT);
      socket.disconnect();
    };
  }, [loadAudit]);
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      // Debounce search input to reduce request noise while typing.
  ...
  const handleDownloadPdf = async () => {
    ...
    setError(null);
    try {
      const pdfBlob = await downloadInspectorWorkflowAuditPdfApi({
        permit,
        search: search.trim(),
        dateFrom,
        dateTo,
      ...
      anchor.download = `inspector-workflow-history-${datePart}.pdf`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      // Revoke with a short delay so browser download handoff is not interrupted.
      window.setTimeout(() => URL.revokeObjectURL(objectUrl), 500);
    } catch (err: unknown) {
      const apiError = err as { response?: { data?: { message?: string } } };
      setError(
        apiError?.response?.data?.message ??
          "Failed to download inspector workflow history PDF.",
      );
    } finally {
  ...
  return (
    <div className="min-h-screen p-2 md:p-6 flex flex-col gap-4">
      <section
        ...
        data-inspector-tour="audit-history-header"
      >
        <h1 className="text-2xl font-black text-[#0F2942]">
          Inspector Workflow History
        </h1>
        <p className="mt-2 text-sm font-medium text-gray-600">
          Workflow events scoped to your assigned inspection actions.
        </p>
          ...
          <label className="lg:col-span-2">
            ...
            <div className="flex items-center rounded-xl border border-gray-200 bg-white px-3">
              ...
              <input
                ...
                placeholder="Search by when, status, actor, permit, applicant, remark..."
      ...
      <section className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
        {rows.length === 0 ? (
          <div className="py-12 px-5 text-center text-sm font-semibold text-gray-500">
            No inspector workflow records found for the current filter.
          </div>
        ) : (
      ...
      <WorkflowAuditDetailsModal
        ...
        event={selectedEvent}
        title="Inspector Workflow Event Details"
