...
import { isInspectionNotification } from "@/utils/owner/owner-status-routing.util";
...
const buildNotificationText = (application: OwnerApplicationStatusType) => {
  ...
  const remark = application.evaluatorRemark?.trim();
  if (!remark) return base;
  return `${base} Remarks: ${remark}`;
};
...
export default function InspectionStatusPage() {
      ...
      try {
        ...
        setError(
          apiError?.response?.data?.message ??
            "Failed to load inspection status records.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const applications = useMemo(
    () => allApplications.filter((item) => isInspectionNotification(item)),
    [allApplications],
  );
  ...
  const handleOpenNotification = async (
    ...
    navigate(`/home/user/inspection-status/${application._id}`);
  };
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      await markAllOwnerApplicationStatusesAsReadApi("inspection");
    } catch {
  ...
  return (
    <OwnerStatusPageShell
      ...
      tourId="status-inspection-header"
      wideDesktop
    >
      {error && (
