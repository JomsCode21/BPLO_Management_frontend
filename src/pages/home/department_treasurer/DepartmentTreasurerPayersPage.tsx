...
export default function DepartmentTreasurerPayersPage() {
  ...
  const loadRecords = async () => {
    ...
    setError(null);
    try {
      const response = await getDepartmentTreasurerPayersApi();
      // Full department payer feed; filtered variants are handled in dedicated pages.
  ...
  useEffect(() => {
    ...
    socket.on(DEPARTMENT_TREASURER_PAYMENT_EVENT, () => {
      // Rehydrate list whenever payment updates are pushed in realtime.
      void loadRecords();
    });
  ...
  return (
    <div className="min-h-screen p-2 md:p-6 space-y-6">
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 md:p-6">
        <h1 className="text-2xl md:text-3xl font-black text-[#0F2942]">List of Payers</h1>
        <p className="text-sm text-gray-500 mt-1">
          Business owners with generated payment assessment in your department.
        </p>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        {sorted.length === 0 ? (
          <div className="py-16 text-center text-sm font-semibold text-gray-500">
            No payers available yet. Only passed inspections with generated assessment appear here.
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-180">
              ...
              <tbody>
                {paginatedRows.map((row) => (
                  <tr key={`${row._id}_${row.departmentId}`} className="border-b border-gray-100">
                    <td className="px-5 py-4">
                      <p className="font-semibold text-[#0F2942]">{row.applicantName}</p>
                      <p className="text-xs text-gray-500">{row.applicantEmail}</p>
                    </td>
                    <td className="px-5 py-4 text-gray-700 font-medium">{row.permitType}</td>
                    <td className="px-5 py-4 text-gray-700 font-medium">{formatDate(row.generatedAt)}</td>
