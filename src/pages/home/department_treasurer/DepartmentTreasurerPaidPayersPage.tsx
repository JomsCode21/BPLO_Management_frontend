...
export default function DepartmentTreasurerPaidPayersPage() {
  ...
  const loadRecords = async () => {
    ...
    setError(null);
    try {
      const response = await getDepartmentTreasurerPayersApi("paid");
      // Defensive filter keeps this page strict even if backend returns mixed states.
      const onlyPaid = (response.data ?? []).filter(
        (row) => normalizeStatus(row.paymentStatus) === "paid",
      );
      setRecords(onlyPaid);
    } catch (err: unknown) {
  ...
  useEffect(() => {
    ...
    socket.on(DEPARTMENT_TREASURER_PAYMENT_EVENT, () => {
      // Refresh paid list immediately after any payment status change event.
      void loadRecords();
    });
    ...
    <div className="min-h-screen p-2 md:p-6 space-y-6">
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 md:p-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Business owners in your department with paid payment status.
        </p>
      </div>
