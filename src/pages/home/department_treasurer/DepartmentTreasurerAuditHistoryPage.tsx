import {
  downloadDepartmentTreasurerWorkflowAuditPdfApi,
  getDepartmentTreasurerWorkflowAuditApi,
...
const initialPagination: DepartmentTreasurerWorkflowAuditPaginationType = {
  page: 1,
...
export default function DepartmentTreasurerAuditHistoryPage() {
  const [rows, setRows] = useState<DepartmentTreasurerWorkflowAuditEventType[]>([]);
  const [pagination, setPagination] =
    useState<DepartmentTreasurerWorkflowAuditPaginationType>(initialPagination);
  ...
  const [selectedEvent, setSelectedEvent] =
    useState<DepartmentTreasurerWorkflowAuditEventType | null>(null);
  ...
  const loadAudit = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      const response = await getDepartmentTreasurerWorkflowAuditApi({
        page,
        limit: ITEMS_PER_PAGE,
        permit,
        search: search.trim(),
      ...
      if (!isSilent) {
        ...
        setError(
          apiError?.response?.data?.message ??
            "Failed to load department workflow history.",
        );
      }
    } finally {
  ...
  useEffect(() => {
    ...
    socket.on(DEPARTMENT_TREASURER_WORKFLOW_AUDIT_EVENT, () => {
      void loadAudit({ silent: true });
    });

    return () => {
      socket.off(DEPARTMENT_TREASURER_WORKFLOW_AUDIT_EVENT);
      socket.disconnect();
    };
  }, [loadAudit]);
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      // Debounce typing to avoid firing an audit request on every key press.
  ...
  const handleDownloadPdf = async () => {
    ...
    setError(null);
    try {
      const pdfBlob = await downloadDepartmentTreasurerWorkflowAuditPdfApi({
        permit,
        search: search.trim(),
        dateFrom,
        dateTo,
      ...
      anchor.download = `department-treasurer-workflow-history-${datePart}.pdf`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      // Delay URL revoke slightly to avoid interrupting browser download start.
      window.setTimeout(() => URL.revokeObjectURL(objectUrl), 500);
    } catch (err: unknown) {
      const apiError = err as { response?: { data?: { message?: string } } };
      setError(
        apiError?.response?.data?.message ??
          "Failed to download department workflow history PDF.",
      );
    } finally {
    ...
    <div className="min-h-screen p-2 md:p-6 flex flex-col gap-4">
      <section className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
        <h1 className="text-2xl font-black text-[#0F2942]">
          Department Payment History
        </h1>
        <p className="mt-2 text-sm font-medium text-gray-600">
          Payment workflow events scoped to your department responsibilities.
        </p>
      ...
      <section className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
        {rows.length === 0 ? (
          <div className="py-12 px-5 text-center text-sm font-semibold text-gray-500">
            No department workflow records found for the current filter.
          </div>
        ) : (
      ...
      <WorkflowAuditDetailsModal
        ...
        event={selectedEvent}
        title="Department Workflow Event Details"
